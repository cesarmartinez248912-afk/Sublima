...
export default function Gallery() {
  ...
  useEffect(() => {
    getSiteConfig().then((cfg) => setGalleryItems(cfg.gallery));
  }, []);
        ...
        >
          <div className="max-w-2xl">
            ...
            <p className="text-[16px] text-on-surface-variant leading-relaxed">
              Explora algunos de nuestros proyectos recientes y encuentra inspiración para tu próxima creación.
            </p>
          </div>
          <button
            ...
            >
              ...
              {!item.imageUrl && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className={`material-symbols-outlined text-primary-container/20 group-hover:text-primary-container/30 transition-all duration-500 group-hover:scale-110 ${item.large ? "text-[100px]" : "text-[60px]"}`}>
                    {item.icon}
                  </span>
                </div>
              )}
              ...
              <div className="absolute bottom-5 left-5 z-20 translate-y-3 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                ...
                {item.large && <h3 className="text-white text-[20px] font-bold mt-1">{item.title}</h3>}
              </div>
              <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
                <div className="bg-white/90 backdrop-blur-sm w-8 h-8 rounded-full flex items-center justify-center">
                  <span className="material-symbols-outlined text-[16px] text-primary-container">open_in_new</span>
                </div>
              </div>
            </motion.div>
          ))}
